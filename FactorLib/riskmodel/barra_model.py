...
class BarraModel(object):
    ...
    def getFactorReturnArgs(self):
        ...
        if not hasattr(self, 'all_industries'):
            all_industries = self.data_source.get_factor_data(args['行业因子'])[args['行业因子']].unique()
            self.all_industries = IndustryConverter.all_values(args['行业因子'])
        if np.nan in self.all_industries:
            self.all_industries.remove(np.nan)
        self.genFactorReturnArgs = args
        self.all_factors = ['Market'] + args['风格因子'] + self.all_industries
        return
